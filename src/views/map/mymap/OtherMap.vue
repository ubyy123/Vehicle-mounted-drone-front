<template>
    <div>
        ...
        <button @click="button(allpoint)"></button>
    </div>
</template>
<script>
    ...
    methods: {
        ...
        button(allpoint) {
            ...
            fetchlat(allpoint).then(response => {
                if (response) {
                    ...
                    for (let j = 0; j < response.obj.uav_route_list.length; ++j) {
                        if (response.obj.uav_route_list[j][0][0] == response.obj.uav_route_list[j][1][0]) {
                            response.obj.uav_route_list[j] = []
                            // console.log(response.obj.uav_route_list[j]);
                        }
                    }
                    that.allpoints = waypoints
                    this.UavLine = response.obj.uav_route_list
                }
            })
        },
        ...
        Go() {
            ...
            for (let i = 0; i < this.UavLine.length; ++i) {
                if (this.UavLine[i].length > 1) {
                    console.log(this.UavLine[i]);
                    uavNumber += 1
                }
            }
            setTimeout(() => {
                that.$store.commit("mapMessage/otherMap", [[distance, time, dis], [carNumber, uavNumber]])
            }, 1000)
            // 绘制无人机飞行路线
        ...
        CarAnimation(path, serialNumber) {
            ...
            marker.on('moving', function (e) {
                ...
                // 判断派送订单数量
                if (flag2) {
                    // console.log(allpoints[serialNumber][i].lng);
